...
def chopEnd(listToChop, lengthToChop):
    if len(listToChop) > lengthToChop:
        return listToChop[:lengthToChop]
    return listToChop
...
def loader():
    ...
    if True:
        training_data_questions = [chopStart(q, 120) for q in training_data_questions]
        val_data_questions = [chopStart(q, 120) for q in val_data_questions]
        training_data_answers = [chopEnd(q, 120) for q in training_data_answers]
        val_data_answers = [chopEnd(q, 120) for q in val_data_answers]
    ...
    all_data['max_enc_len'] = max(enc_lengths)
    all_data['max_dec_len'] = max(dec_lengths)
